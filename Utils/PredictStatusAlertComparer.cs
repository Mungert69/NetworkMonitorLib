...
public class PredictStatusAlertComparer : IEqualityComparer<PredictStatusAlert>
{
    public bool Equals(PredictStatusAlert? x, PredictStatusAlert? y)
    {
        if (ReferenceEquals(x, y)) return true;
        if (ReferenceEquals(x, null) || ReferenceEquals(y, null)) return false;

        // Include comparisons for any additional properties from PredictStatus as needed
        return x.ID == y.ID &&
               x.UserID == y.UserID &&
               x.Address == y.Address &&
    ...
    public int GetHashCode(PredictStatusAlert obj)
    {
