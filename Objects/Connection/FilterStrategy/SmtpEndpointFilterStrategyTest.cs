...
public class SmtpEndpointFilterStrategyTest
{
    ...
    public void ShouldInclude_CorrectlyCyclesThroughEndpoints()
    {
        ...
        int n = 3;
        var strategy = new SmtpEndpointFilterStrategy(n);
        var endpoints = new List<INetConnect>();
        for (int i = 0; i < 6; i++)
        {
            endpoints.Add(new TestNetConnect
            {
                MpiStatic = new MPIStatic { EndPointType = "smtp", MonitorIPID = i }
            });
        }
    ...
    public void ShouldInclude_CorrectlyCyclesThroughEndpoints_WithNonZeroOffset()
    {
        ...
        int n = 3;
        int offset = 2;
        var strategy = new SmtpEndpointFilterStrategy(n, 0, offset);
        var endpoints = new List<INetConnect>();
        for (int i = 0; i < 6; i++)
        {
            endpoints.Add(new TestNetConnect
    ...
    {
        var strategy = new SmtpEndpointFilterStrategy(3);
