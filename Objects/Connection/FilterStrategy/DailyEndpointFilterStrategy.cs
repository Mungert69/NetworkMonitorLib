...
namespace NetworkMonitor.Connection
{
    public class DailyEndpointFilterStrategy : INetConnectFilterStrategy, IEndpointSettingStrategy
    {
        ...
        private readonly List<string> _matchingConnectNames = new() { "daily" };

        // Thread-safe dictionary for last connect times
        private readonly ConcurrentDictionary<int, DateTime> _lastConnectTimes = new();

        public void SetTotalEndpoints(List<INetConnect> netConnects)
        {
            _totalEndpoints = netConnects.Count(x =>
                _matchingConnectNames.Any(connectName =>
                    x.MpiStatic.EndPointType.ToLower().Contains(connectName))
            );
        }

        public bool ShouldInclude(INetConnect netConnect)
        {
            if (_matchingConnectNames.Any(connectName =>
                    netConnect.MpiStatic.EndPointType.ToLower().Contains(connectName)))
            {
                var now = DateTime.UtcNow;

                // Spread connects by hour of day (24 slots)
                const int slotsPerDay = 24;
...
/*               Console.WriteLine(
                ...
                {
                    // Mark as run for today (thread-safe write)
